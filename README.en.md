...
The initial Filter Builder runner exposes `BrowserFFmpeg.ffmpegFilterBuilderArgs(...)` and accepts compiled `--video-filter` / `--audio-filter` chains. It intentionally does not claim full multi-input `filter_complex` support yet; that remains a later Filter Builder application milestone.
...
A v1.9.4 tag rebuilds and smoke-tests the existing seven ST profiles plus both FFmpeg Filter Builder variants. The release publishes separate `ffmpeg-wasm-ffmpeg-filter-builder-single-thread-v1.9.4.zip` and `ffmpeg-wasm-ffmpeg-filter-builder-multi-thread-v1.9.4.zip` assets, BUILDINFO files, SHA-256 checksums, and one exact corresponding-source archive.
