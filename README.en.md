...
The Filter Builder runner 0.2.1 treats `trim` / `atrim` filter-source EOF as successful bounded completion and probes the compiled video graph before encoder creation. As a result, long-input previews finalize normally and dimension-changing filters such as `scale`, `crop`, `pad`, and `transpose` are preserved in the encoded output.
