...
```text
git tag -a v1.1.0 -m "FFmpeg WASM Builder v1.1.0"
git push origin v1.1.0
```

The tag workflow verifies that the tag matches `BUILDER_VERSION`, rebuilds both current release profiles, runs their real browser smoke tests, then publishes:
