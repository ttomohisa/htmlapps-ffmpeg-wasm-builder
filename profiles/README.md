...
`ffmpeg-filter-builder` declares `PROFILE_THREADING_VARIANTS="single-thread,multi-thread"` and `PROFILE_PTHREAD_POOL_SIZE=8`, `PROFILE_DECODER_THREAD_COUNT=2`, `PROFILE_ENCODER_THREAD_COUNT=4`, and `PROFILE_X264_LOOKAHEAD_THREAD_COUNT=1`. It is intentionally staged: the runner accepts real video/audio filter chains, bounded time-range rendering, and H.264/AAC MP4 output, while full multi-input execution remains a later milestone. v1.9.8 adds drawtext with FreeType/HarfBuzz so the app can provide its own embedded local font.
