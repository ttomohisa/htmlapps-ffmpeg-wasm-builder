...
Current release profiles include `video-compressor`, `lossless-video-cutter`, `media-inspector`, and `video-contact-sheet`. Media Inspector demonstrates a read-only JSON-report runner, while Video Contact Sheet demonstrates seek-based frame sampling + RGB conversion without an image encoder. Both use WORKERFS for large browser File/Blob input.
